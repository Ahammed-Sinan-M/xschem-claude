...
# RDW batch additions (2026-09-05)

## You share one file with four other items

Every item in this batch edits `src/rdw.tcl`. **Items run strictly one at a
time for that reason** — if you find yourself wanting to start the next one
while yours is still open, don't. Two crews in that file at once is the
sabotage-on-a-copy trap from above, wearing a different hat.

## The three suites that must not move

`test_rdw_window_1245` (109), `test_rdw_keys_1245` (41, needs a display),
`test_op_param_store_1245` (130). Report per case if anything moves, in either
direction. `test_op_annot` (485) is the control.

## `test_rdw_keys_1245` has a known flake and it is a TEST defect

Issue **1332**: rows SD1/SD2/SD3b arm a modal driver on a fixed `after 100`
rather than polling for `.rdw.scope`. Observed once in 134 runs under display
contention. Measured margin is 3–6 ms against a 100 ms timer, max 19 ms over 88
runs. If you see it, re-run; if you have spare capacity, **fix it by polling**
— do not widen the delay.

## Two filed-not-fixed defects sit under this window

* **1330** — `rdw::_apply_now` swallows an `apply` failure while the status line
  reports success. Item R2 is the first thing that makes that channel
  load-bearing. Fix it or say loudly that you did not.
* **1331** — the narrow arm refuses a symbol path containing a space in the
  store's own internal jargon, and unlike its two siblings offers no
  *"Choose every device of class X instead."*

## A green suite is not an eyeball, and this batch is nearly all pixels

Four of the five items are things the user will judge by looking: a shade of
grey, a raised window, a selection highlight, a number's format. **Record a
`look` debt** (`tests/headless/owed.sh add look …`) and say *"suites green,
please look"*. Never report one of these done on a green suite.
